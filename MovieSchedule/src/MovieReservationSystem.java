...
/**
 * This class calls all the function necessary for adding movies, & creating and
...
public class MovieReservationSystem {
    ...
    /**
     * Constructor. Includes setting up the values of the system and calling the
     * focal functions.
     *
     * @author ajruth.sumandang
     */
    ...
    /**
     * Display the movies in the system sorted by date. Only display the first
     * top ten recent movies unless the user chose to show more of the movie
     * schedules.
     *
     * @author ajruth.sumandang
     */
    private void displayMovieSchedules() {
        int i = 0;
        Comparator<MovieSchedule> comp = (m1, m2) -> {
            return m1.getStartDate().compareTo(m2.getStartDate());
        };
        Collections.sort(MovieScheduleTest.schedules, comp);
        if (MovieScheduleTest.schedules.size() == 0) {
        ...
        while (MovieScheduleTest.schedules.size() > i) {
            int movieCounter=0;
            if (MovieScheduleTest.schedules.get(i).isIsActive()) {
                movieCounter++;
                if(movieCounter==0){System.out.println("No scheduled movies yet in the system. Please add movies.");}
                System.out.print("[" + movieCounter + "] ");
                System.out.print("[" + (i + 1) + "] ");
                MovieSchedule m = MovieScheduleTest.schedules.get(i);
            ...
            i++;
            if (i != 0 && i % 10 == 0) {
                System.out.println(i + " out of " + MovieScheduleTest.schedules.size() + " movies");
    ...
    private void cancelMovieSchedule() {
        ...
        try {
            ...
            MovieScheduleTest.schedules.get(choice - 1).deactivate();
            MovieScheduleTest.schedules.remove(choice - 1);
        } catch (InvalidInputException e) {
            System.out.println(e);
     ...
     *
     * @author ajruth.sumandang
     * @throws InvalidInputException if the user input an invalid value and
     * chooses not to proceed when prompted
     */
    ...
    /**
     * Holds the function necessary for editing movie information. Includes the
     * user's interface and edits the received movie information
     *
     * @param movie - the movie to be edited
     * @return the edited movie; null if the user chose not to create the movie
     ...
     *
     * @param movieSched - the movie schedule whose information is to be
     * displayed
     *
     * @author ajruth.sumandang
     */
    ...
    /**
     * Sets up the system by initializing the variables obtained from the
     * database.
     *
     * @author ajruth.sumandang
     */
